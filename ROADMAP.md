...
**ВАЖНОЕ**
- [x] поправить прокрутку. Это может быть вызвано как lenis, так и чем бы то ни было еще, но на xiaomi телефонах, на тачпадах асусов и еще неизвестном ряде устройств прокрутка не работает вообще. От футера выпадашки мы избавляемся, поэтому в принципе можно найти более щадящие способы прокрутки.
...
- [X] `docs` переснять скриншоты в readme/ — устарели после редизайна (РУЧНОЕ — нужен браузер)
...
- [x] `infra` **www не редиректится, корень двух пунктов отчёта.** `https://www.lyoraeth.art/` отдавал 200 и полноценный HTML (canonical при этом честно указывал на apex) — www висел вторым доменом на том же Proxy Host в Nginx Proxy Manager, физически проксировался в то же приложение. Убран из Domain Names основного хоста, заведён отдельным Redirection Host (https, Preserve Path, 301) + свой SSL-сертификат. Проверено на проде: `www.lyoraeth.art/work` → `301` → `lyoraeth.art/work` (путь сохраняется)
...
- [x] `infra` **`/ru/` и `/ru` обе отдавали 200**, редиректа между ними не было, canonical у обеих на `/ru`. Не ошибка (Google выбор уважает), но краулинговый бюджет тратился. Фикс — `location = /ru/ { return 301 /ru; }` на nginx, НЕ через Nuxt `routeRules` (проверено 07.09.2026: Nitro/rou3 нормализует слеш при матчинге routeRules-ключей, `/ru/` и `/ru` считаются одним и тем же паттерном — правило `'/ru/': redirect('/ru')` цепляло и голый `/ru` тоже, уходило в бесконечный редирект-луп). Проверено на проде: `/ru/` → `301` → `/ru`, голый `/ru` остался 200
...
- [x] `infra` **РУЧНОЕ НА VPS — шрифты редизайна.** Лицензионные woff2 (PP Neue Montreal Medium, PP Pangram Sans Semibold) в репозиторий не коммитятся (`/public/fonts/` в .gitignore), значит в Docker-образ они не попадают. Сделано: `server/routes/fonts/[file].ts` отдаёт их с VPS через bind-mount мимо публичной статики, `Cache-Control: immutable`, `Cross-Origin-Resource-Policy: same-origin` (закрывает хотлинк с чужих доменов). Проверено на проде — шрифт отдаётся 200 в DevTools Network
