...
- Images: AVIF/WebP via Sanity CDN, `loading="lazy"` everywhere except above-the-fold covers; cover dimensions pulled from Sanity asset metadata and passed as explicit `width`/`height` — zero CLS
...
### Security
All routes carry `Strict-Transport-Security` (preload), `Cross-Origin-Opener-Policy: same-origin`, and `X-Frame-Options: DENY`. A `Content-Security-Policy-Report-Only` header is in place with a verified allowlist; it will be promoted to enforced once stable in production.
