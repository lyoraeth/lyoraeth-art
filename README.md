...
- `llms-full.txt` — dynamic Nitro route that fetches all work and writing from Sanity at request time and renders them as plain text (PortableText → Markdown). Cached for 1 h with `stale-while-revalidate`. Always up to date — no build-time generation step needed.
